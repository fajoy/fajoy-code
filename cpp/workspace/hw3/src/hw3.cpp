...
char send_buf[4096];
int send_len;
void set_send_buf(const char *c,int size){
	if(size>0){
	memcpy(&send_buf[send_len],c,size);
	send_len+=size;
	}
}
...
void init_req() {
	...
	if (query == NULL) {
		...
		//query = testquery;
		char testquery[] ="h1=127.0.0.1&p1=8001&f1=t1.txt"
				"&h2=&p2=&f2="
				"&h3=&p3=&f3="
				"&h4=&p4=&f4="
				"&h5=&p5=&f5=";
...
public:
	...
	FILE *fp;
	void parse(char *shost, char* str_port, char *sfilename, char* ssocks,char * ssport) {
		strcpy(host, shost);
		strcpy(socks, ssocks);
	...
	void parse(char *shost, char* str_port, char *sfilename) {
		strcpy(host, shost);
		strcpy(filename, sfilename);
		port = atoi(str_port);
	}
	...
	int readfile() {
		if (is_openfile) {
			...
			if (fgets(buf, sizeof(buf), fp) != NULL) {
				int len = strlen(buf);
				if (is_con)
					s.setSendData((unsigned char *)buf,len);
				return 1;
			} else {
				is_openfile = false;
				//write_buf = "";
...
void js(char *line,int index) {
		...
		sprintf(buf,"<script type='text/javascript'>document.all['m%d'].innerHTML += \"",index);
		set_send_buf(buf,strlen(buf));

		set_send_buf(tmp.c_str(),tmp.length());
	...
	void bjs(char *line,int index) {
		string tmp = line;
			...
			char buf[255];
			memset(buf,0,sizeof(buf));
			sprintf(buf,"<script type='text/javascript'>document.all['m%d'].innerHTML += \"<b>",index);
			set_send_buf(buf,strlen(buf));

			set_send_buf(tmp.c_str(),tmp.length());

			memset(buf,0,sizeof(buf));
			sprintf(buf,"</b>\";</script>\n");
			set_send_buf(buf,strlen(buf));
...
void select_con() {
	...
	while (sc[0].s.getMode()>= 0
		...
		SerCon *c = &sc[i];
		mySocket *s = &sc[i].s;
		//count <<"i"<<i<<fds.isRSet();

		if(fds.isRSet(s->socket_fd) ){
					s->recvData();
					if (s->hasRecvData) {
							js((char *)&s->recv_buffer[0],i);
							s->clear_recv_data();
							fds.setW(1);
							if(s->getMode() ==0){
								s->setMode(1);
		...
		else if (fds.isWSet(s->socket_fd) && s->getMode() == 1&&c->isCanSend()) {
			s->sendData();
			if(s->hasSendData){
				if(s->sendError){
...
void con_Sc() {
	...
	for (i = 0; i < 5; i++) {
		...
		if (hasHost && hasFile && hasPort) {
			sc[i].s.remote_host=sc[i].host;
			sc[i].s.remote_port=sc[i].port;

			if(sc[i].s.connect_socket()==0){
